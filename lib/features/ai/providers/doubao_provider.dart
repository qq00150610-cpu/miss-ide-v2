...
class DoubaoProvider implements AIProvider {
  String? _apiKey;
  String _baseUrl = AIConstants.doubaoEndpoint;
  final Map<String, UsageStats> _usageStats = {};

  DoubaoProvider({String? apiKey, String? baseUrl}) {
    _apiKey = apiKey;
  ...
  @override
  AIProviderType get providerType => AIProviderType.doubao;

  @override
  List<AIModel> get supportedModels => AIModelCatalog.getModelsForProvider(AIProviderType.doubao);
  ...
  Future<bool> isAvailable() async {
    ...
    try {
      ...
      logger.e(LogTags.ai, 'Doubao availability check failed', error: e);
      return false;
    }
  }
  ...
  Stream<AIResponse> complete(
    ...
    final selectedModel = modelId ?? 'doubao-pro-4k';
    
    final prompt = '''请补全以下代码中光标位置的代码：
...
请直接输出补全代码：''';
  ...
  Stream<AIResponse> explain(
    ...
    final selectedModel = modelId ?? 'doubao-pro-32k';
...
```''';
  ...
  Stream<AIResponse> _sendRequest({
    ...
    try {
      ...
      logger.e(LogTags.ai, 'Doubao request failed', error: e);
      yield AIErrorResponse(
        model,
        '请求失败: ${e.toString()}',
